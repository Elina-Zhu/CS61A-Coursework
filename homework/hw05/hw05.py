class VendingMachine:
    ...
    def vend(self):
        if not self.quota:
            ...
            self.funds = 0
            return (
                f"Here is your {self.item} and ${change} change."
                if change
                else f"Here is your {self.item}."
            )

    def restock(self, amount):
        self.quota += amount
        return f"Current {self.item} stock: {self.quota}"
...
class Coin:
    ...
    def worth(self):
        return (
            self.cents + Mint.current_year - self.year - 50
...
def store_digits(n):
    ...
    """
    def helper(n, last_link=Link.empty):
        if n < 10:
            return Link(n, last_link)
        return helper(n // 10, Link(n % 10, last_link))
    return helper(n)
