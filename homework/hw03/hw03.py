...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert not upper_bound(x) == 0
    assert not upper_bound(y) == 0
...
def multiple_references_explanation():
    return """She is right because, in par1, each interval is referenced twice, 
    causing the interval to become looser as different values are chosen for each instance of the interval. 
    In par2, each interval is referenced only once, resulting in a tighter interval."""
