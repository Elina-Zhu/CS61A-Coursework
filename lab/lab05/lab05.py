...
def closer_city(lat, lon, city_a, city_b):
    ...
    y1 = get_lon(city_a)
    y2 = get_lon(city_b)
    distance_a = sqrt((x1 - lat)**2 + (y1 - lon)**2)    
    distance_b = sqrt((x2 - lat)**2 + (y2 - lon)**2)
...
def add_trees(t1, t2):
    ...
    ...                            [tree(7)]),
    ...                       tree(8)])])
    >>> print_tree(add_trees(numbers, numbers)) # Case 1
    2
      4
        6
        8
      10
        12
          14
        16
    >>> print_tree(add_trees(tree(2), tree(3, [tree(4), tree(5)]))) # Case 2
    5
      4
      5
    >>> print_tree(add_trees(tree(2, [tree(3)]), tree(2, [tree(3), tree(4)]))) # Case 3
    4
      6
      4
    ...
    if t1 is None and t2 is None:
        ...
        value = label(t1) + label(t2)
        return tree(value, [add_trees(b1, b2) for b1, b2 in zip_longest(branches(t1), branches(t2))])
